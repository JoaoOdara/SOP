...
function parseFaturamentoXLSX(data) {
  // data = array of rows (array of arrays from SheetJS)
  const meses = [];
  // Row 7 (index 7): detect month names in columns 2,4,6
  const monthRow = data[7] || [];
  const dataRow  = data[8] || []; // headers
  
  // Identify month columns from row 7
  const monthCols = [];
  for (let c = 2; c < monthRow.length - 2; c += 2) {
    const v = String(monthRow[c] || '').trim();
    if (v && !v.includes('Total') && v !== '') {
  ...
  for (let r = 9; r < data.length - 1; r++) { // skip last "Total geral" row
    const row = data[r] || [];
    const cod = String(row[1] || '').trim();
    if (!cod || cod === '') {
      if (row[0]) familiaAtual = String(row[0]).trim();
      continue;
    }
    if (!cod.startsWith('PP') && !cod.startsWith('MP')) {
      if (row[0]) familiaAtual = String(row[0]).trim();
      continue;
    }
    
    const sku = { codigo: cod, familia: familiaAtual || '', meses: {} };
    monthCols.forEach(mc => {
      const r_val = parseFloat(String(row[mc.colR] || '0').replace(',','.')) || 0;
      const q_val = parseFloat(String(row[mc.colQ] || '0').replace(',','.')) || 0;
      sku.meses[mc.nome] = { receita: r_val, qtd_cx: q_val };
    });
    sku.total_cx     = parseFloat(String(row[data[8].length-1] || '0')) || 0;
    sku.total_receita= parseFloat(String(row[data[8].length-2] || '0')) || 0;
    skus[cod] = sku;
  }
