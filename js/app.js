...
function parsePlanoComercial(rows) {
  ...
  for (let i = 0; i < Math.min(5, rows.length); i++) {
    const r = rows[i] || [];
    if (r.some(c => String(c||'').toUpperCase().trim() === 'CÓD' || String(c||'').toUpperCase().trim() === 'COD')) {
      headerIdx = i; break;
    }
    if (r.some(c => String(c||'').toUpperCase().includes('PRODUTO'))) {
      headerIdx = i; break;
    }
  }
  ...
  const iCod  = header.findIndex(h => h === 'CÓD' || h === 'COD' || h === 'CÓDIGO' || h === 'CODIGO');
  const iProd = header.findIndex(h => h.includes('PRODUTO'));
  const iMes  = header.findIndex(h => h === 'MES' || h === 'MÊS');
  const iTipo = header.findIndex(h => h === 'TIPO');
  const iVlr  = header.findIndex(h => h === 'VLR' || h === 'VALOR');
  const iQtd  = header.findIndex(h => h === 'QTD' || h === 'QUANTIDADE');

  console.info('[parsePlano] Header at row', headerIdx, '→ COD:', iCod, 'PROD:', iProd, 'MES:', iMes, 'TIPO:', iTipo, 'VLR:', iVlr, 'QTD:', iQtd);

  if (iCod < 0 || iMes < 0 || iTipo < 0) {
    console.error('[parsePlano] Could not find required columns');
    return null;
  }

  const skus = {};  // { 'PP01000': { codigo, produto, meses: { 1: {prev_qtd, prev_vlr, real_qtd, real_vlr}, ... } } }

  for (let r = headerIdx + 1; r < rows.length; r++) {
    const row = rows[r] || [];
    const cod = String(row[iCod]||'').trim().toUpperCase();
    if (!cod || !cod.match(/^PP/)) continue;

    const produto = iProd >= 0 ? String(row[iProd]||'').trim() : '';
    const tipo = String(row[iTipo]||'').trim().toUpperCase();
    const vlr = parseFloat(String(row[iVlr]||'0').replace(',','.')) || 0;
    const qtd = parseFloat(String(row[iQtd]||'0').replace(',','.')) || 0;

    // Parse month from date or number
    let mes = 0;
    const mesVal = row[iMes];
    if (mesVal instanceof Date) {
      mes = mesVal.getMonth() + 1;
    } else if (typeof mesVal === 'number') {
      // Excel serial date
      const d = new Date((mesVal - 25569) * 86400000);
      mes = d.getMonth() + 1;
    } else {
      const s = String(mesVal||'').toLowerCase();
      ...
      mes = mMap[s] || parseInt(s) || 0;
      if (!mes && mesVal) {
        // Try parsing as date string
        const dt = new Date(mesVal);
        if (!isNaN(dt)) mes = dt.getMonth() + 1;
      }
    }
    if (mes < 1 || mes > 12) continue;

    if (!skus[cod]) skus[cod] = { codigo: cod, produto: produto, meses: {} };
    if (!skus[cod].meses[mes]) skus[cod].meses[mes] = { prev_qtd:0, prev_vlr:0, real_qtd:0, real_vlr:0 };

    if (tipo.includes('PREV')) {
      skus[cod].meses[mes].prev_qtd = Math.round(qtd);
      skus[cod].meses[mes].prev_vlr = vlr;
    } else if (tipo.includes('REAL') || tipo.includes('AJUS')) {
      skus[cod].meses[mes].real_qtd = Math.round(qtd);
      skus[cod].meses[mes].real_vlr = vlr;
    }
  }

  PLAN_CACHE = { skus, parsed_at: new Date().toISOString() };
  console.info('[parsePlano]', Object.keys(skus).length, 'SKUs parsed');
  return PLAN_CACHE;
}
...
async function savePlanoToSupabase(planData) {
  ...
  if (!isDbReady()) return false;
  const rows = [];
  Object.values(planData.skus).forEach(s => {
    Object.entries(s.meses).forEach(([mes, v]) => {
      rows.push({
        chave: 'plan_' + s.codigo + '_' + mes,
        valor: JSON.stringify({ cod: s.codigo, prod: s.produto, mes: parseInt(mes),
          prev_qtd: v.prev_qtd, prev_vlr: v.prev_vlr, real_qtd: v.real_qtd, real_vlr: v.real_vlr }),
      });
    });
  });
...
async function loadPlanoFromSupabase() {
  ...
  rows.forEach(r => {
    try {
      ...
      if (!v.cod) return;
      if (!skus[v.cod]) skus[v.cod] = { codigo: v.cod, produto: v.prod || '', meses: {} };
      skus[v.cod].meses[v.mes] = { prev_qtd: v.prev_qtd||0, prev_vlr: v.prev_vlr||0, real_qtd: v.real_qtd||0, real_vlr: v.real_vlr||0 };
    } catch(e) {}
  });
  if (!Object.keys(skus).length) return null;
  PLAN_CACHE = { skus, parsed_at: new Date().toISOString(), from_db: true };
  console.info('[loadPlano]', Object.keys(skus).length, 'SKUs from Supabase');
  return PLAN_CACHE;
}
