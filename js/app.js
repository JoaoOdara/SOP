...
async function dbSelect(table, opts) {
  ...
  try {
    ...
    return data || [];
  } catch(e) { console.error('dbSelect exception', table, e); return []; }
}
...
function setChartDefaults() {
  ...
  // Ensure grid colors adapt to theme
  var gridClr = t === 'light' ? 'rgba(0,0,0,0.06)' : 'rgba(255,255,255,0.05)';
...
function gridColorLight() {
  return getTheme() === 'light' ? 'rgba(0,0,0,0.04)' : 'rgba(255,255,255,0.03)';
}
function metaLineColor() {
  return getTheme() === 'light' ? 'rgba(0,0,0,0.15)' : 'rgba(255,255,255,0.2)';
}
...
const METAS_DEFAULT = {
  ...
  otd_dias: 3,       // On Time Delivery — dias úteis limite
...
async function saveCapConfig(cfg) {
  ...
  Object.entries(cfg).forEach(([linha, vals]) => {
    Object.entries(vals).forEach(([campo, val]) => {
      rows.push({ chave: `cap_${linha}_${campo}`, valor: String(val) });
    });
  });
  const ok = await dbUpsert('sop_config_geral', rows, 'chave');
...
async function saveFatToSupabase(fatData) {
  ...
  console.info('[saveFat] Saving', rows.length, 'rows for ano='+ano);
  try {
    // Delete old data
    const { error: delErr } = await db.from('sop_vendas_semanais').delete().eq('ano', ano);
    if (delErr) console.warn('[saveFat] Delete error (may be ok if empty):', delErr.message);
    
    // Insert in batches of 50 to avoid payload limits
    let inserted = 0;
    for (let i = 0; i < rows.length; i += 50) {
      const batch = rows.slice(i, i + 50);
      const { error: insErr } = await db.from('sop_vendas_semanais').insert(batch);
      if (insErr) {
        console.error('[saveFat] Insert error at batch '+Math.floor(i/50)+':', insErr.message);
        console.error('[saveFat] Sample row:', JSON.stringify(batch[0]));
        showToast('Erro ao salvar faturamento: ' + insErr.message, 'error', 8000);
        return false;
      }
      inserted += batch.length;
    }
    console.info('[saveFat] SUCCESS:', inserted, 'rows inserted');
    return true;
  } catch(e) {
    console.error('[saveFat] Exception:', e);
    showToast('Erro ao salvar faturamento: ' + e.message, 'error', 8000);
    return false;
  }
}
...
async function loadFatFromSupabase(ano) {
  ...
  console.info('[loadFat] Querying sop_vendas_semanais for ano='+ano);
  const rows = await dbSelect('sop_vendas_semanais', { eq: { ano }, order: 'mes', asc: true });
  console.info('[loadFat] Got', rows.length, 'rows');
  if (!rows.length) {
    // Try without year filter to see if data exists at all
    const allRows = await dbSelect('sop_vendas_semanais', { limit: 5 });
    if (allRows.length) {
      console.warn('[loadFat] Data exists but not for ano='+ano+'. Sample:', allRows[0]);
      // Try with the year from the data
      const dataAno = allRows[0].ano;
      if (dataAno && dataAno !== ano) {
        console.info('[loadFat] Retrying with ano='+dataAno);
        const retryRows = await dbSelect('sop_vendas_semanais', { eq: { ano: dataAno }, order: 'mes', asc: true });
        if (retryRows.length) {
          console.info('[loadFat] Found', retryRows.length, 'rows for ano='+dataAno);
          return buildFatCache(retryRows);
        }
      }
    }
    return null;
  ...
  return buildFatCache(rows);
}

function buildFatCache(rows) {
  ...
  console.info('[buildFatCache]', Object.keys(skus).length, 'SKUs,', mesesNomes.join(', '));
  return FAT_CACHE;
}

// ── PLANO COMERCIAL (sop_plano_comercial) ─────────────────────
// Parsed from "Projeção de vendas ajustada.xlsx"
// Format: Base_Completa sheet with CÓD | PRODUTO | MES | TIPO(PREV/REAL_AJUS) | VLR | QTD
let PLAN_CACHE = null;

function parsePlanoComercial(rows) {
  // Format: Pivot table
  // Col A: "LINHA 1" / "LINHA 2" / "LINHA 3" / "TOTAL"
  // Col B: Product name OR "PREV" / "REAL/AJUS" / "QTD AJUS" / "TOTAL LINHA X"
  // Cols C-N: Monthly values (dates or month numbers)
  // Each product has 3 rows: PREV (R$), REAL/AJUS (R$), QTD AJUS (units)

  // Detect month columns from first product row
  var monthCols = [];
  for (var ri = 0; ri < Math.min(5, rows.length); ri++) {
    var r = rows[ri] || [];
    for (var ci = 2; ci < Math.min(16, r.length); ci++) {
      var v = r[ci];
      if (!v) continue;
      var mes = 0;
      if (v instanceof Date || (typeof v === 'object' && v)) {
        mes = new Date(v).getMonth() + 1;
      } else if (typeof v === 'number' && v > 40000 && v < 50000) {
        // Excel serial date
        mes = new Date((v - 25569) * 86400000).getMonth() + 1;
      } else {
        var s = String(v).toLowerCase();
        if (s.includes('total')) continue;
        var dateTest = new Date(v);
        if (!isNaN(dateTest) && dateTest.getFullYear() >= 2025) mes = dateTest.getMonth() + 1;
      }
      if (mes >= 1 && mes <= 12) monthCols.push({ col: ci, mes: mes });
    }
    if (monthCols.length >= 10) break;
  }

  console.info('[parsePlano] Found', monthCols.length, 'month columns');
  if (monthCols.length < 6) {
    console.error('[parsePlano] Not enough month columns found');
    return null;
  }
  ...
  for (var r = 0; r < rows.length; r++) {
    ...
    var colBup = colB.toUpperCase();

    // Detect line group
    if (colA.startsWith('LINHA')) {
      currentLinha = colA;
    }
    if (colA === 'TOTAL' || colBup.startsWith('TOTAL')) {
      currentProduct = null;
      continue;
    }

    // Detect product name (col B has a product name, not PREV/REAL/QTD)
    if (colB && !['PREV', 'REAL/AJUS', 'QTD AJUS', 'REAL/AJUSTADO'].includes(colBup) &&
        !colBup.startsWith('TOTAL') && colBup.length > 5 &&
        colBup.includes('ALFAJOR') || colBup.includes('MINI') || colBup.includes('DUPLO') || colBup.includes('PEANUT') ||
        colBup.includes('CROCANTE') || colBup.includes('ZERO') || colBup.includes('MARÉ') || colBup.includes('CHAVES') || colBup.includes('BROWNIE')) {
      currentProduct = colB;
      if (!skus[currentProduct]) {
        skus[currentProduct] = { produto: colB, linha: currentLinha, meses: {} };
        monthCols.forEach(function(mc) {
          skus[currentProduct].meses[mc.mes] = { prev_vlr: 0, real_vlr: 0, qtd_ajus: 0 };
        });
      }
      continue;
    }

    if (!currentProduct || !skus[currentProduct]) continue;

    // Parse data rows
    if (colBup === 'PREV') {
      monthCols.forEach(function(mc) {
        var val = parseFloat(String(row[mc.col] || '0').replace(',', '.')) || 0;
        skus[currentProduct].meses[mc.mes].prev_vlr = val;
      });
    } else if (colBup === 'REAL/AJUS' || colBup === 'REAL/AJUSTADO') {
      monthCols.forEach(function(mc) {
        var val = parseFloat(String(row[mc.col] || '0').replace(',', '.')) || 0;
        skus[currentProduct].meses[mc.mes].real_vlr = val;
      });
    } else if (colBup === 'QTD AJUS') {
      monthCols.forEach(function(mc) {
        var val = parseFloat(String(row[mc.col] || '0').replace(',', '.')) || 0;
        skus[currentProduct].meses[mc.mes].qtd_ajus = Math.round(val);
      });
    }
  }

  // Clean: remove products with all zeros
  Object.keys(skus).forEach(function(k) {
    var s = skus[k];
    var hasData = Object.values(s.meses).some(function(m) { return m.qtd_ajus > 0 || m.prev_vlr > 0 || m.real_vlr > 0; });
    if (!hasData) delete skus[k];
  });

  PLAN_CACHE = { skus: skus, parsed_at: new Date().toISOString() };
  console.info('[parsePlano]', Object.keys(skus).length, 'SKUs parsed from pivot format');
  return PLAN_CACHE;
}

async function savePlanoToSupabase(planData) {
  PLAN_CACHE = planData;
  if (!isDbReady()) return false;
  const rows = [];
  Object.entries(planData.skus).forEach(([nome, s]) => {
    Object.entries(s.meses).forEach(([mes, v]) => {
      rows.push({
        chave: 'plan_' + nome.substring(0,30).replace(/[^a-zA-Z0-9]/g,'_') + '_' + mes,
        valor: JSON.stringify({ prod: nome, linha: s.linha, mes: parseInt(mes),
          prev_vlr: v.prev_vlr, real_vlr: v.real_vlr, qtd_ajus: v.qtd_ajus }),
      });
    });
  });
  // Save as sop_config_geral entries with plan_ prefix
  const ok = await dbUpsert('sop_config_geral', rows, 'chave');
  if (ok) logChange('Dados', 'Plano comercial carregado', Object.keys(planData.skus).length + ' SKUs');
  return ok;
}

async function loadPlanoFromSupabase() {
  const rows = await dbSelect('sop_config_geral', { like: { chave: 'plan_%' } });
  ...
  rows.forEach(r => {
    try {
      ...
      if (!v.prod) return;
      if (!skus[v.prod]) skus[v.prod] = { produto: v.prod, linha: v.linha || '', meses: {} };
      skus[v.prod].meses[v.mes] = { prev_vlr: v.prev_vlr||0, real_vlr: v.real_vlr||0, qtd_ajus: v.qtd_ajus||0 };
    } catch(e) {}
  });
  if (!Object.keys(skus).length) return null;
  PLAN_CACHE = { skus, parsed_at: new Date().toISOString(), from_db: true };
  console.info('[loadPlano]', Object.keys(skus).length, 'products from Supabase');
  return PLAN_CACHE;
}
...
async function saveEstoqueToSupabase(parsed, nomeArquivo) {
  ...
  const ok = await dbUpsert('sop_estoque_semanal', rows, 'sku_codigo,semana_ref');
  console.info('[saveEstoque] Result:', ok ? 'SUCCESS' : 'FAILED', rows.length, 'rows');
  return ok;
}
...
async function loadEstoqueFromSupabase() {
  ...
  ESTOQUE_CACHE = rows.map(r => ({
    sku:        r.sku_codigo,
    desc:       r.descricao || r.sku_codigo || '',
    estoque_cx: r.estoque_caixas || 0,
    pedidos_cx: r.pedidos_caixas || 0,
    un_cx:      r.un_por_cx || 72,
    data_ref:   r.semana_ref,
  }));
  console.info('[loadEstoque]', ESTOQUE_CACHE.length, 'SKUs loaded, ref:', dataRef);
  return { rows: ESTOQUE_CACHE, data_ref: dataRef };
}

// ── UPLOAD TIMESTAMPS ─────────────────────────────────────────
// Synchronous cache — populated at startup and after uploads
const _uploadTsCache = {};

// SYNC: always returns string or null (from cache, never a Promise)
function getUploadTs(tipo) {
  return _uploadTsCache[tipo] || null;
}

// Async: populate cache from Supabase at startup
async function loadUploadTimestamps() {
  if (!isDbReady()) return;
  try {
    const { data } = await db.from('sop_uploads')
      .select('tipo, criado_em')
      .order('criado_em', { ascending: false });
    if (data) {
      // Keep only latest per tipo
      const seen = {};
      data.forEach(r => { if (r.tipo && !seen[r.tipo]) { seen[r.tipo]=r.criado_em; _uploadTsCache[r.tipo]=r.criado_em; } });
    }
  } catch(e) { console.warn('loadUploadTimestamps:', e); }
}

// SYNC format helper (safe — ts is always a string or null)
function formatUploadTs(tipo) {
  const ts = getUploadTs(tipo);
  return ts ? formatTs(ts) : '—';
}

// Called after a successful upload to update both cache and DB
async function setUploadTs(tipo, nomeArquivo, totalSkus) {
  const now = new Date().toISOString();
  _uploadTsCache[tipo] = now;
  // Update sidebar sync display
  const el = document.getElementById('meta-sync');
  if (el) el.textContent = formatTs(now);
  if (!isDbReady()) return false;
  return await dbUpsert('sop_uploads', {
    tipo, arquivo_nome: nomeArquivo || tipo,
    semana_ref: now.split('T')[0],
...
async function saveCalendario(dias) {
  const rows = dias.slice(1).map((v, i) => ({ chave: `cal_${i+1}`, valor: String(v) }));
  const ok = await dbUpsert('sop_config_geral', rows, 'chave');
...
async function saveSazonalidade(saz) {
  const rows = saz.slice(1).map((v, i) => ({ chave: `saz_${i+1}`, valor: String(v) }));
  const ok = await dbUpsert('sop_config_geral', rows, 'chave');
...
async function saveExpedicao(cfg) {
  const rows = Object.entries(cfg).map(([k,v]) => ({ chave:`exp_${k}`, valor:String(v) }));
  return await dbUpsert('sop_config_geral', rows, 'chave');
}

async function saveAvancado(cfg) {
  const rows = Object.entries(cfg).map(([k,v]) => ({ chave:`adv_${k}`, valor:String(v) }));
  return await dbUpsert('sop_config_geral', rows, 'chave');
}
...
const NAV = [
  ...
  { group:'DECISÃO' },
  { id:'cenarios',    href:'cenarios.html',    icon:'⚖', label:'Cenários'               },
  { id:'portfolio',   href:'portfolio.html',   icon:'◰', label:'Portfólio'              },
  { id:'acoes',       href:'acoes.html',       icon:'✓', label:'Ações'                  },
  { group:'SISTEMA' },
...
async function initApp(activeId) {
  ...
  // Load from Supabase — silently fall back to defaults on any error
  if (isDbReady()) {
    try {
      await Promise.all([
        loadMetas(),
        ...
        loadHEConfig(),
        loadCalendario(),
        loadUploadTimestamps(),
        loadFatFromSupabase(),
        loadEstoqueFromSupabase(),
        loadPlanoFromSupabase(),
      ]);
      console.info('[initApp] Supabase data loaded:', {
        metas: Object.keys(METAS).length+' keys',
        fat: FAT_CACHE ? Object.keys(FAT_CACHE.skus).length+' SKUs' : 'empty',
        est: ESTOQUE_CACHE ? ESTOQUE_CACHE.length+' rows' : 'empty',
      });
    } catch(e) {
      console.warn('initApp: Supabase load error (using defaults):', e);
    }
  }
