...
function parseFaturamentoXLSX(data) {
  // Structure (Omie "Vendas Empresa por Produto"):
  //   Row 0-4: title/filters
  //   Row 6: "Data de Emissão.Mês" label
  //   Row 7: month names at cols 3,5,7... (every 2, starting col 3); col 9 = "Totais"
  //   Row 8: headers — col0=Família, col1=Código, col2=Descrição, col3+= data pairs
  //   Row 9+: data rows (col0=família group header OR empty when SKU row)
  //   Last row: "Total geral"

  // 1. Find month row (row with "Janeiro" or month name)
  let monthRowIdx = -1;
  const MONTH_NAMES = ['janeiro','fevereiro','marco','abril','maio','junho',
                       'julho','agosto','setembro','outubro','novembro','dezembro'];
  for (let i = 0; i < Math.min(12, data.length); i++) {
    const r = data[i] || [];
    if (r.some(c => MONTH_NAMES.includes(String(c||'').toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g,'').trim()))) {
      monthRowIdx = i;
      break;
    }
  }
  if (monthRowIdx < 0) monthRowIdx = 7; // fallback

  const monthRow = data[monthRowIdx] || [];

  // 2. Detect month columns — scan all cells in monthRow for month names
  const monthCols = [];
  for (let c = 0; c < monthRow.length; c++) {
    const v = String(monthRow[c] || '').trim();
    const vn = v.toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g,'');
    if (MONTH_NAMES.includes(vn)) {
      monthCols.push({ nome: v, colR: c, colQ: c + 1 });
    }
  }

  // 3. Find header row (row with "Código do Produto" or "Código")
  let headerRowIdx = monthRowIdx + 1;
  for (let i = monthRowIdx; i < Math.min(monthRowIdx + 4, data.length); i++) {
    const r = data[i] || [];
    if (r.some(c => String(c||'').toLowerCase().includes('código'))) {
      headerRowIdx = i;
      break;
    }
  }
  ...
  for (let r = dataStartIdx; r < data.length; r++) {
    const row = data[r] || [];
    // Skip empty rows and "Total geral"
    if (!row.some(c => c !== '' && c != null)) continue;
    if (String(row[0]||'').toLowerCase().includes('total geral')) continue;

    const col0 = String(row[0] || '').trim();
    const col1 = String(row[1] || '').trim();
    const col2 = String(row[2] || '').trim();

    // Family header row: col0 has text, col1 has SKU code OR is empty
    if (col0 && col0.startsWith('PP') === false && col0.startsWith('MP') === false) {
      // Could be family label (e.g. "PP - CLASSICO") — update familiaAtual
      if (!col1.match(/^[A-Z]{2}\d{2,}/) || col0.length > 3) {
        familiaAtual = col0;
      }
    }

    // SKU row: col1 must be a valid SKU code (e.g. PP01001, MP00001)
    const cod = col1 || col0;
    if (!cod.match(/^[A-Z]{2}\d{2,}/)) continue;

    // Familia: col0 might have the family if this is first SKU of group
    if (col0 && col0 !== cod && col0.length > 3) familiaAtual = col0;
    ...
    monthCols.forEach(mc => {
      const rVal = parseFloat(String(row[mc.colR] || '0').replace(',','.')) || 0;
      const qVal = parseFloat(String(row[mc.colQ] || '0').replace(',','.')) || 0;
      sku.meses[mc.nome] = { receita: rVal, qtd_cx: qVal };
    });

    // Totals: last 2 columns
    const lastCol = row.length - 1;
    sku.total_cx      = parseFloat(String(row[lastCol]   || '0').replace(',','.')) || 0;
    sku.total_receita = parseFloat(String(row[lastCol-1] || '0').replace(',','.')) || 0;

    // Only keep if has any sales data
    if (sku.total_cx > 0 || sku.total_receita > 0 ||
        Object.values(sku.meses).some(m => m.receita > 0 || m.qtd_cx > 0)) {
      skus[cod] = sku;
    }
  }
